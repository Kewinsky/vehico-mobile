...
import { listServiceEntries, createServiceEntry } from "../services/serviceEntries/serviceEntriesRepo";
...
export function DataPortabilityScreen({ navigation, route }: Props) {
  ...
  async function onImportServiceEntries() {
    try {
      ...
      if (iDate < 0 || iTitle < 0) throw new Error(t("dataPortability.importMissingColumns"));
      ...
      const toCreate = rows
        .map((r) => ({
          ...
          mileage: iMileage >= 0 && (r[iMileage] ?? "").length ? Number(r[iMileage]) : null,
          cost: iCost >= 0 && (r[iCost] ?? "").length ? Number(r[iCost]) : null,
        }))
        .filter((r) => r.service_date.trim().length === 10 && r.title.trim().length);

      if (toCreate.length === 0) throw new Error(t("dataPortability.importNothing"));
              ...
              try {
                ...
                for (const e of toCreate) {
                  await createServiceEntry({
                    ...
                    mileage: Number.isFinite(e.mileage as any) ? (e.mileage as any) : null,
                    title: e.title.trim(),
                    description: (e.description ?? "").trim(),
                    cost: Number.isFinite(e.cost as any) ? (e.cost as any) : null,
                  });
                }
                setCsv("");
  ...
  return (
    <FormScreen header={<AppHeader onBack={() => navigation.goBack()} />}>
      ...
      <Button onPress={onExport} disabled={exporting || importing}>
        {exporting ? t("common.loading") : t("dataPortability.exportAction")}
      </Button>
      ...
      <TextField
        ...
        editable={!importing && !exporting}
      />
      <View style={{ height: 12 }} />
      <Button onPress={onImportServiceEntries} disabled={importing || exporting}>
